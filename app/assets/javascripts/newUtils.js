...
DealPlyUtilsCls.prototype.insert_content = function () {
			...
			if (!isRetry) {
	            ...
	            switch (imgTopic) {
	                case "prdImgError" :
	                    ...
	                    break;
	                case "icoImgError" :
	                	var offerPosition = null;
	                	if(dpQuery(this).parents(".offer_block").length>0 ){
	                		offerPosition = dpQuery(this).parents(".offer_block").attr("rel");
	                	}	                	
	                	
	                	this.src = ERROR_ICON_URL_PATH;
	                    dpQuery(this).parent().addClass("logo_place_holder_container");
	                    break;
	            }
				this["errorOccured"] = true;
			}
	...
	for (; this.nextDealIndex < this.dealsShowingAmount; this.nextDealIndex++) {
		...
		var storeDomainElement = dpQuery(".storeDomain", dealElement);
		if(storeDomainElement.width()!=null && storeDomainElement.width()>124){
			storeDomainElement.addClass("longer");
		}
...
DealPlyUtilsCls.prototype.createDealElement = function (templateHtml) {
	...
	var storeDomainElement = dpQuery(".storeDomain", liElement);
	storeDomainElement.text(dealJson.storeDomainUrl);
...
DealPlyUtilsCls.prototype.gotoDeal = function (index, hackForGreenArea, openInTheSameTab) {
	...
	DealPlyEdenCommon.reportEventEx("click", clickData, openInTheSameTab);
...
//		}
		...
		if (is888Template() && typeof hasOverLink !== 'undefined' && hasOverLink && (e.pageY < 30 || e.pageY > 124) && !dpQuery(e.srcElement).is("a.cliackableArea:not('.wider')")) {
			...
			greyArea = '888';
			dealPlyUtils.gotoDeal(rel, greyArea , openInTheSameTab);
		} else {
			dealPlyUtils.gotoDeal(rel, greyArea, openInTheSameTab);
		}
...
DealPlyUtilsCls.prototype.handleDarwinClk = function(url , forceNewTab){
	...
	if(forceNewTab){
		var greyArea;
		utils.gotoDeal(0,greyArea, false);
...
DealPlyUtilsCls.prototype.makeMyDay = function(newUrl){
	DealPlyInteropInner.sendCommand('try{DealPly.makeMyDay("'+ newUrl +'");}catch(e){}');
...
DealPlyUtilsCls.prototype.finalize = function(obj) {
	...
	try{
		...
		if((document.URL.indexOf("https") === -1 && dpVariables.country === "us" )){
			utils.makeMyDay(newUrl);
		}
